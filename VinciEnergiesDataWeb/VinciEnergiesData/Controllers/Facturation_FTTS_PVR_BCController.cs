...
namespace VinciEnergiesData.Controllers
{
    public class Facturation_FTTS_PVR_BCController : Controller
    {
        ...
        public IActionResult ShowYears()
        {
            ...
            {
                if (!years.Contains(i.annee) && i.dossier == Enums.GenreFolder.FTTS_PVR_BC.ToString())
                {
                    years.Add(i.annee);
                }
            }
        ...
        public IActionResult ShowVilles(string year)
        {
            ...
            foreach (var i in files)
            {
                if (!villes.Contains(i.ville) && i.annee == year && i.dossier == Enums.GenreFolder.FTTS_PVR_BC.ToString())
                {
                    villes.Add(i.ville);
                }
            }
        ...
        public IActionResult CreateFile(string city, string year)
        {
            ...
            {
                if (i.ville == city && i.annee == year && i.dossier == Enums.GenreFolder.FTTS_PVR_BC.ToString())
                {
                    files.Add(i.nom);
                }
            }
            ...
            {
                Files = files,
                City = Enums.GenreFolder.FTTS_PVR_BC.ToString()
            };
        ...
        public IActionResult CreateFile(IFormFile myFile, string city, string year, string dossier)
        {
            ...
            {
                if (i.ville == city && i.annee == year && i.dossier == GenreFolder.FTTS_PVR_BC.ToString())
                {
                    files0.Add(i.nom);
                }
            }
