...
namespace VinciEnergiesData.Controllers
{
    public class Facturation_B2B_PartageFONController : Controller
    {
        ...
        public IActionResult ShowYears()
        {
            ...
            {
                if (!years.Contains(i.annee) && i.dossier == Enums.GenreFolder.B2B_PartageFON.ToString())
                {
                    years.Add(i.annee);
                }
            }
        ...
        public IActionResult ShowVilles(string year)
        {
            ...
            foreach (var i in files)
            {
                if (!villes.Contains(i.ville) && i.annee == year && i.dossier == Enums.GenreFolder.B2B_PartageFON.ToString() && i.annee == year)
                {
                    villes.Add(i.ville);
                }
            }
        ...
        public IActionResult CreateFile(string city, string year)
        {
            ...
            {
                if (i.ville == city && i.annee == year && i.dossier == Enums.GenreFolder.B2B_PartageFON.ToString())
                {
                    files.Add(i.nom);
                }
            }
            ...
            {
                Files = files,
                City = Enums.GenreFolder.B2B_PartageFON.ToString()
            };
        ...
        public IActionResult CreateFile(IFormFile myFile, string city, string year, string dossier)
        {
            ...
            {
                if (i.ville == city && i.annee == year && i.dossier == GenreFolder.B2B_PartageFON.ToString())
                {
                    files0.Add(i.nom);
                }
            }
