...
{
    public class Facturation_FTTS_PartageGCController : Controller
    {
        private readonly ApplicationDbContext _db;
        private readonly string wwwrootDirectory = Path.Combine(Directory.GetCurrentDirectory(), "wwwroot/Files");

        public Facturation_FTTS_PartageGCController(ApplicationDbContext db)
        {
            _db = db;
        }
        ...
        public IActionResult CreateFile(IFormFile myFile, string city, string year, string dossier)
        {
            ...
            {
                if (i.ville == city && i.annee == year && i.dossier == GenreFolder.FTTS_PartageGC.ToString())
                {
                    files0.Add(i.nom);
                }
            }
