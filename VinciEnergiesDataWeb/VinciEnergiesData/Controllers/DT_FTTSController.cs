...
namespace VinciEnergiesData.Controllers
{
    ...
    public class DT_FTTSController : Controller
    {
        ...
        public DT_FTTSController(ApplicationDbContext db)
        {
            _db = db;
        }
        ...
        public IActionResult ShowYears()
        {
            ...
            {
                if (!years.Contains(i.annee) && i.genre == Enums.GenreFolder.FTTS && i.folder == Enums.FolderType.DossierTechnique)
                {
                    years.Add(i.annee);
                }
            }
        ...
        public IActionResult ShowVilles(string year)
        {
            ...
            foreach (var i in files)
            {
                if (!villes.Contains(i.ville) && i.annee == year && i.genre == Enums.GenreFolder.FTTS && i.folder == Enums.FolderType.DossierTechnique)
                {
                    villes.Add(i.ville);
                }
            }
        ...
        public IActionResult ShowFolders(string city, string year)
        {
            ...
            foreach (var i in files)
            {
                if (i.ville == city && i.annee == year && i.genre == Enums.GenreFolder.FTTS && i.folder == Enums.FolderType.DossierTechnique)
                {
