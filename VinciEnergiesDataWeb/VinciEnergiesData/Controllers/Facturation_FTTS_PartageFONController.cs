...
{
    public class Facturation_FTTS_PartageFONController : Controller
    {
        private readonly ApplicationDbContext _db;
        private readonly string wwwrootDirectory = Path.Combine(Directory.GetCurrentDirectory(), "wwwroot/Files");

        public Facturation_FTTS_PartageFONController(ApplicationDbContext db)
        {
            _db = db;
        }
        ...
        public IActionResult ShowYears()
        {
            ...
            {
                if (!years.Contains(i.annee) && i.dossier == Enums.GenreFolder.FTTS_PartageFON.ToString())
                {
                    years.Add(i.annee);
                }
            }
        ...
        public IActionResult ShowVilles(string year)
        {
            ...
            foreach (var i in files)
            {
                if (!villes.Contains(i.ville) && i.annee == year && i.dossier == Enums.GenreFolder.FTTS_PartageFON.ToString())
                {
                    villes.Add(i.ville);
                }
            }
        ...
        public IActionResult CreateFile(string city, string year)
        {
            ...
            {
                if (i.ville == city && i.annee == year && i.dossier == Enums.GenreFolder.FTTS_PartageFON.ToString())
                {
                    files.Add(i.nom);
                }
            }
            ...
            {
                Files = files,
                City = Enums.GenreFolder.FTTS_PartageFON.ToString()
            };
        ...
        public IActionResult CreateFile(IFormFile myFile, string city, string year, string dossier)
        {
            ...
            {
                if (i.ville == city && i.annee == year && i.dossier == GenreFolder.FTTS_PartageFON.ToString())
                {
                    files0.Add(i.nom);
                }
            }
