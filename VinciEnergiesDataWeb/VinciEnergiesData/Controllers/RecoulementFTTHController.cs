...
{
    [Authorize]
    public class RecoulementFTTHController : Controller
    {
        private readonly ApplicationDbContext _db;
        private readonly string wwwrootDirectory = Path.Combine(Directory.GetCurrentDirectory(), "wwwroot/Files");

        public RecoulementFTTHController(ApplicationDbContext db)
        {
            _db = db;
        }
        ...
        public IActionResult ShowYears()
        {
            ...
            {
                if (!years.Contains(i.annee) && i.genre == Enums.GenreFolder.FTTH && i.folder == Enums.FolderType.Recoulement)
                {
                    years.Add(i.annee);
                }
            }
        ...
        public IActionResult ShowVilles(string year)
        {
            ...
            foreach (var i in files)
            {
                if (!villes.Contains(i.ville) && i.annee == year && i.genre == Enums.GenreFolder.FTTH && i.folder == Enums.FolderType.Recoulement)
                {
                    villes.Add(i.ville);
                }
            }
        ...
        public IActionResult ShowFolders(string city, string year)
        {
            ...
            foreach (var i in files)
            {
                if (i.ville == city && i.annee == year && i.genre == Enums.GenreFolder.FTTH && i.folder == Enums.FolderType.Recoulement)
                {
